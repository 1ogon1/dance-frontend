...
const baseUrl = "https://dance-backend-b9e62d6b3dc5.herokuapp.com/api";
...
export const vote = async (values, battleId, participantId) => {
  ...
  return await axios.post(
    `${baseUrl}/battles/vote/${battleId}/${participantId}`,
    values,
    options
  );
};
...
export const phoenixPower = async (participantId) => {
  ...
  return await axios.post(
    `${baseUrl}/battles/phoenixPower/` + JSON.parse(participantId),
    options,
    options
  );
};
...
export const setWinner = async (values, battleId) => {
  ...
  return await axios.post(
    `${baseUrl}/battles/setWinner/` + battleId,
...
export const resetBattle = async (values, battleId) => {
  ...
  return await axios.post(
    `${baseUrl}/battles/reset/` + battleId,
    JSON.stringify(values),
    options
  );
};
